...
static unsigned char acute[] =
    "AÁEÉIÍOÓUÚYÝaáeéiíoóuúyýSŚZŹsśzźRŔLĹCĆNŃrŕlĺcćnń";
...
static unsigned char macron[] =
    "AĀDĐEĒHĦIĪOŌTŦUŪaādđeēhħiīoōtŧuū"; /* and stroke! */
...
static unsigned char latin2[] = {" Ą˘Ł¤ĽŚ§¨ŠŞŤŹ­ŽŻ°ą˛ł´ľśˇ¸šşťź˝žż"
                                 "ŔÁÂĂÄĹĆÇČÉĘËĚÍÎĎĐŃŇÓÔŐÖ×ŘŮÚŰÜÝŢß"
                                 "ŕáâăäĺćçčéęëěíîďđńňóôőö÷řůúűüýţ˙"};
static unsigned char latin3[] = {" Ħ˘£¤  Ĥ§¨İŞĞĴ­  Ż°ħ²³´µĥ·¸ışğĵ½  ż"
                                 "ÀÁÂ  ÄĊĈÇÈÉÊËÌÍÎÏ  ÑÒÓÔĠÖ×ĜÙÚÛÜŬŜß"
                                 "àáâ  äċĉçèéêëìíîï  ñòóôġö÷ĝùúûüŭŝ˙"};
static unsigned char latin4[] = {" ĄĸŖ¤ĨĻ§¨ŠĒĢŦ­Ž¯°ą˛ŗ´ĩļˇ¸šēģŧŊžŋ"
                                 "ĀÁÂÃÄÅÆĮČÉĘËĖÍÎĪĐŅŌĶÔÕÖ×ØŲÚÛÜŨŪß"
                                 "āáâãäåæįčéęëėíîīđņōķôõö÷øųúûüũū˙"};
static unsigned char latin5[] = {" ¡¢£¤¥¦§¨©ª«¬­®¯°±²³´µ¶·¸¹º»¼½¾¿"
                                 "ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏĞÑÒÓÔÕÖ×ØÙÚÛÜİŞß"
                                 "àáâãäåæçèéêëìíîïğñòóôõö÷øùúûüışÿ"};
static unsigned char latin6[] = {" ĄĒĢĪĨĶ§ĻĐŠŦŽ­ŪŊ°ąēģīĩķ·ļđšŧž―ūŋ"
                                 "ĀÁÂÃÄÅÆĮČÉĘËĖÍÎÏÐŅŌÓÔÕÖŨØŲÚÛÜÝÞß"
                                 "āáâãäåæįčéęëėíîïðņōóôõöũøųúûüýþĸ"};
static unsigned char *latin_tb[5] = {latin2, latin3, latin4, latin5, latin6};
...
static void txt_add_cnv(unsigned char *s, int sz, int comment) {
  ...
  while (sz > 0) {
    ...
    case '\\':
      if (sz >= 4 /* \ooo */
          && p[1] >= '0' && p[1] <= '3' && p[2] >= '0' && p[2] <= '7' &&
          p[3] >= '0' && p[3] <= '7') {
        c = ((p[1] - '0') << 6) + ((p[2] - '0') << 3) + p[3] - '0';
        if (p != s)
          txt_add(s, (int)(p - s));
        p += 4;
        s = p;
...
static void txt_add_eos(char *fname, int linenum) {
  ...
  if (parse.abc_vers == (2 << 16) && offset > 0 && dst[offset - 1] == '\\') {
    offset--;
    return;
  }
  txt_add(&eos, 1);
...
static int tune_select(unsigned char *s) {
  ...
  ret = regcomp(&r, (char *)sel, REG_EXTENDED | REG_NEWLINE | REG_NOSUB);
  if (ret)
    return 0;
...
/* -- front end parser -- */
void frontend(unsigned char *s, int ftype, char *fname, int linenum) {
  ...
  if (ftype == FE_ABC && strncmp((char *)s, "%abc-", 5) == 0) {
    get_vers((char *)s + 5);
    while (*s != '\0' && *s != '\r' && *s != '\n')
      s++;
    if (*s != '\0') {
  ...
  if (ftype == FE_ABC &&
    ...
    for (p = s; *p != '\0'; p++) {
      ...
      if (c == '\\') {
        if (!isdigit(p[1]))
          continue;
        if ((p[1] == '0' || p[1] == '2') && p[2] == '0') /* accidental */
          continue;
        latin = 1;
        break;
      }
  ...
  while (*s != '\0') {
    ...
    if (begin_end) {
      if (ftype == FE_FMT) {
        if (strncmp((char *)s, "end", 3) == 0 &&
            strncmp((char *)s + 3, (char *)begin_end, end_len) == 0) {
          begin_end = NULL;
          goto next_eol;
        }
        if (*s == '%')
      ...
      if (*s == '%' && strchr(prefix, s[1])) {
        ...
        if (strncmp((char *)q, "end", 3) == 0 &&
            strncmp((char *)q + 3, (char *)begin_end, end_len) == 0) {
          begin_end = NULL;
          goto next_eol;
        }
      }
    ...
    if (l == 0) { /* empty line */
      ...
      default:
        goto ignore;
      case 1:
        fprintf(stderr, "Line %d: Empty line in tune header - K:C added\n",
                linenum);
    ...
    if (histo) { /* H: continuation */
      if ((s[1] == ':' && isalpha(*s)) || (*s == '%' && strchr(prefix, s[1]))) {
        histo = 0;
      } else {
        if (*s != '+' || s[1] != ':')
    ...
    pcinfo:
      ...
      if (strncmp((char *)s, "format ", 7) == 0 ||
        ...
        *q = '\0';
        skip_sav = skip;
        // fixme: pb when different encoding in included file: != behaviour .fmt
        // or .abc... 				latin_sav = latin;
        offset = 0;
        include_file(s);
        //				latin = latin_sav;
        skip = skip_sav;
  ...
  info:
    if (s[1] == ':' && (isalpha(*s) || *s == '+')) {
      ...
      case 'X':
        switch (pstate) {
        case 1:
          fprintf(stderr, "Line %d: X: found in tune header - K:C added\n",
                  linenum);
          txt_add((unsigned char *)"K:C", 3);
          txt_add_eos(fname, linenum);
  ...
  if (ftype == FE_FMT)
    return;
  if (pstate == 1)
    fprintf(stderr, "Line %d: Unexpected EOF in header definition\n", linenum);
  abc_eof();
}
