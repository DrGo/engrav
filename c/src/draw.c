...
static void draw_note(float x, struct SYMBOL *s, int fl);
...
void set_color(int new_color) {
  if (new_color == cur_color)
    return;
  cur_color = new_color;
  a2b("%.2f %.2f %.2f setrgbcolor ", (float)(cur_color >> 16) / 255,
      (float)((cur_color >> 8) & 0xff) / 255, (float)(cur_color & 0xff) / 255);
}
...
static float b_pos(int grace, int stem, int flags, float b) {
...
static int calculate_beam(struct BEAM *bm, struct SYMBOL *s1) {
  struct SYMBOL *s, *s2;
  ...
  static float min_tb[2][6] = {
      ...
      {STEM_CH_MIN, STEM_CH_MIN, STEM_CH_MIN2, STEM_CH_MIN3, STEM_CH_MIN4,
       STEM_CH_MIN4}};
  ...
  for (s2 = s1;; s2 = s2->next) {
    if (s2->abc_type == ABC_T_NOTE) {
      ...
      if (s2->stem != s1->stem)
        two_dir = 1;
      if (!visible && !(s2->flags & ABC_F_INVIS) &&
          (!(s2->flags & ABC_F_STEMLESS) || (s2->sflags & S_TREM2)))
        visible = 1;
    ...
    if (!s2->next) { /* beam towards next music line */
      ...
      s->u.note.slur_end = 0;
      s->x += 12;
      if (s->x < state.realwidth - 12)
        s->x = state.realwidth - 12;
      s2 = s;
      notes++;
      break;
    }
  ...
  if (staff_tb[staff].y == 0) { /* staves not defined */
    ...
    if (!two_staves) {
      ...
      bm->b = s1->ys - s1->xs * bm->a + staff_tb[staff].y;
      bm->nflags = nflags;
      return 1;
    }
  }
  ...
  if (two_dir) { /* 2 directions */
    ...
    while (s->ts_prev->abc_type == ABC_T_NOTE && s->ts_prev->time == s->time &&
           s->ts_prev->x > s1->xs)
      s = s->ts_prev;

    for (; s && s->time <= s2->time; s = s->ts_next) {
      if (s->abc_type != ABC_T_NOTE || (s->flags & ABC_F_INVIS) ||
          (s->staff != staff && s->voice != voice)) {
        continue;
      }
    ...
    for (s = s1->next;; s = s->next) {
      ...
      case NOTEREST: /* cannot move rests in multi-voices */
        if (s->abc_type != ABC_T_REST)
          break;
        g = s->ts_next;
        if (!g || g->staff != staff || g->type != NOTEREST)
          break;
  ...
  if (a == 0) /* shift flat beams onto staff lines */
    b +=
        b_pos(s1->flags & ABC_F_GRACE, s1->stem, nflags, b - staff_tb[staff].y);
...
//fixme: hack
    ...
    case ABC_T_REST:
      ...
      dy = BEAM_DEPTH + BEAM_SHIFT * (nflags - 1) + (s->head != H_FULL ? 4 : 9);
      if (s1->stem > 0) {
        y -= dy;
        if (s1->multi == 0 && y > 12)
          y = 12;
...
static void draw_beam(float x1, float x2, float dy, float h, struct BEAM *bm,
                      int n) /* beam number (1..n) */
{
  ...
  s = bm->s1;
  if ((s->sflags & S_TREM2) && n > s->nflags - s->aux && s->head != H_EMPTY) {
    if (s->head >= H_OVAL) {
      x1 = s->x + 6;
      x2 = bm->s2->x - 6;
    } else {
...
static void draw_beams(struct BEAM *bm) {
  ...
  if (cur_trans == 0 && cur_scale != 1) {
    bm->a /= cur_scale;
    bm->b = s1->ys - s1->xs * bm->a + staff_tb[s1->staff].y;
    bshift *= cur_scale;
  }
  ...
  da = 0;
  for (s = s1;; s = s->next) {
    if (s->abc_type == ABC_T_NOTE && s->stem != beam_dir)
      s->ys = bm->a * s->xs + bm->b - staff_tb[s->staff].y +
  ...
  for (i = 2; i <= bm->nflags; i++) {
    ...
    for (s = s1;; s = s->next) {
      ...
      if (s->abc_type != ABC_T_NOTE || s->nflags < i) {
        if (s == s2)
          break;
        continue;
      }
      if ((s->sflags & S_TREM1) && i > s->nflags - s->aux) {
        if (s->head >= H_OVAL)
          x1 = s->x;
        else
          x1 = s->xs;
      ...
      if (k1 == k2) {
        if (k1 == s1) {
          ...
          while (k->abc_type != ABC_T_NOTE)
            k = k->next;
          if ((k->sflags & S_BEAM_BR1) || ((k->sflags & S_BEAM_BR2) && i > 2)) {
            x1 -= bstub;
          } else {
            k1 = k1->prev;
...
static void draw_sysbra(float x, int staff, int flag) {
  ...
  yt = staff_tb[staff].y + staff_tb[staff].topbar * staff_tb[staff].staffscale;
  yb = staff_tb[end].y + staff_tb[end].botbar * staff_tb[end].staffscale;
  a2b("%.1f %.1f %.1f %s\n", yt - yb, x, yt,
...
static void draw_lstaff(float x) {
  ...
  a2b("%.1f %.1f %.1f bar\n",
      staff_tb[i].y + staff_tb[i].topbar * staff_tb[i].staffscale - yb, x, yb);
  for (i = 0; i <= nst; i++) {
...
/* -- draw a staff -- */
static void draw_staff(int staff, float x1, float x2) {
  char *stafflines;
  int i, l, thick = -1;
  float y, w;
...
static void draw_timesig(float x, struct SYMBOL *s) {
  ...
  for (i = 0; i < s->u.meter.nmeter; i++) {
    ...
    if (s->u.meter.meter[i].bot[0] != '\0') {
      sprintf(meter, "(%.8s)(%.2s)", s->u.meter.meter[i].top,
      ...
      switch (s->u.meter.meter[i].top[0]) {
      case 'C':
      ...
      case '(':
      case ')':
        sprintf(meter, "(\\%s)", s->u.meter.meter[i].top);
        f = "stsig";
        break;
      default:
        sprintf(meter, "(%.8s)", s->u.meter.meter[i].top);
        f = "stsig";
        break;
      }
    if (meter[0] != '\0')
...
static void draw_acc(int acc, int microscale) {
  ...
  if (acc >> 3 != 0 && microscale) {
    ...
    if (acc >> 3 != 0 && cfmt.nedo) // %%MIDI temperamentequal <nedo>
...
// draw helper lines
static void draw_hl(float x, float staffb, int up, int y, char *stafflines,
                    char *hltype) {
  int i, l;
...
static void draw_keysig(struct VOICE_S *p_voice, float x, struct SYMBOL *s) {
  int old_sf = s->aux;
  ...
  if (s->u.key.nacc == 0 && !s->u.key.empty) {
    ...
    if (cfmt.cancelkey || s->u.key.sf == 0) {
      ...
      if (s->u.key.sf == 0 || old_sf * s->u.key.sf < 0) {
    ...
    for (i = 0; i < s->u.key.nacc; i++) {
      ...
      last_acc = acc;
      if (shift < 0)
        draw_hl(x, staffb, 0, shift, /* lower ledger line */
                staff_tb[s->staff].stafflines, "hl");
      else if (shift > 24)
        draw_hl(x, staffb, 1, shift, /* upper ledger line */
                staff_tb[s->staff].stafflines, "hl");
      last_shift = shift;
      putxy(x, staffb + shift);
...
static void draw_rest(struct SYMBOL *s) {
  ...
  char *stafflines;
  float x, dotx, staffb;
  static char *rest_tb[NFLAGS_SZ] = {"r128", "r64", "r32", "r16", "r8",
                                     "r4",   "r2",  "r1",  "r0",  "r00"};
  ...
  if (s->sflags & S_REPEAT) {
    ...
    if (s->doty < 0) {
      ...
      a2b("mrep\n");
      if (s->doty > 2 && s->voice == cursys->top_voice) {
        /*fixme				set_font(s->gcf); */
        set_font(cfmt.anf);
        putxy(x, staffb + 24 + 4);
        a2b("M(%d)showc\n", s->doty);
  ...
  a2b("%s ", s->u.note.notes[0].head ? s->u.note.notes[0].head : rest_tb[i]);
...
static void draw_gracenotes(struct SYMBOL *s) {
  ...
  if (voice_tb[s->voice].key.instr == K_HP /* no slur when bagpipe */
      || voice_tb[s->voice].key.instr == K_Hp || options.pipeformat ||
      !cfmt.graceslurs || s->u.note.slur_st /* explicit slur */
      || !s->next || s->next->abc_type != ABC_T_NOTE)
    return;
  last = g;
  if (last->stem >= 0) {
    yy = 127;
  ...
  a2b("%.2f %.2f %.2f %.2f %.2f %.2f ", x1 - x0, y1 - y0, x2 - x0, y2 - y0,
      x3 - x0, y3 - y0);
...
static void setdoty(struct SYMBOL *s, signed char *y_tb) {
  int m, m1, y, doty;
...
/* (the staves are defined) */
static void draw_basic_note(float x, struct SYMBOL *s, int m,
                            signed char *y_tb) {
   ...
   * and note on a line */
  if (y % 6 == 0 && shhd != (s->stem > 0 ? s->u.note.notes[0].shhd
...
static void draw_note(float x, struct SYMBOL *s, int fl) {
  ...
  if (!(s->flags & ABC_F_INVIS)) {
    ...
    shhd =
        (s->stem > 0 ? s->u.note.notes[0].shhd : s->u.note.notes[s->nhd].shhd) *
        cur_scale;
    if (s->pits[0] < 22)
  ...
  if (!(s->flags & ABC_F_INVIS) && fl && (s->sflags & S_TREM1)) {
    float x1;
    ...
    if (s->head >= H_OVAL) {
      ...
      x1 += ((s->flags & ABC_F_GRACE) ? GSTEM_XOFF : STEM_XOFF) * s->stem;
      if (s->stem > 0)
        slen += 6 + 5.4 * s->aux;
      else
        slen -= 6 + 5.4;
...
static struct SYMBOL *next_scut(struct SYMBOL *s) {
  ...
  for (s = s->next; s; s = s->next) {
    if (s->type == BAR &&
        ((s->sflags & S_RRBAR) || s->u.bar.type == B_THIN_THICK ||
         s->u.bar.type == B_THICK_THIN ||
         (s->u.bar.repeat_bar && s->text && s->text[0] != '1')))
      return s;
    prev = s;
  }
...
static int slur_direction(struct SYMBOL *k1, struct SYMBOL *k2) {
  struct SYMBOL *s;
  int some_upstem, low;
...
/* -- output a slur / tie -- */
static void slur_out(float x1, float y1, float x2, float y2, int s,
                     float height, int dotted,
                     int staff) /* if < 0, the staves are defined */
{
  ...
  if (!dotted)
    a2b("%.2f %.2f %.2f %.2f %.2f %.2f 0 %.2f ", (xx2 - dx - x2) / cur_scale,
        (yy2 + dy - y2 - dz) / scale_y, (xx1 + dx - x2) / cur_scale,
        (yy1 + dy - y2 - dz) / scale_y, (x1 - x2) / cur_scale,
        (y1 - y2 - dz) / scale_y, dz);
  a2b("%.2f %.2f %.2f %.2f %.2f %.2f ", (xx1 - x1) / cur_scale,
      (yy1 - y1) / scale_y, (xx2 - x1) / cur_scale, (yy2 - y1) / scale_y,
...
static int slur_multi(struct SYMBOL *k1, struct SYMBOL *k2) {
  for (;;) {
...
static int draw_slur(struct SYMBOL *k1_orig, struct SYMBOL *k2, int m1, int m2,
                     int slur_type) {
  struct SYMBOL *k1, *k;
  ...
  if (two_staves)
    error(0, k1, "*** multi-staves slurs not treated yet");
  ...
  if (m1 >= 0) {
    ...
    if (k1->abc_type == ABC_T_NOTE) {
      if (s > 0) {
        ...
        if (k1->stem < 0) {
          x1 -= 1;
          if ((k1->sflags & S_BEAM_END) && k1->nflags >= -1 &&
              (!(k1->sflags & S_IN_TUPLET) || k1->ys < y1 + 3)) {
            if (k1->nflags > 0) {
              x1 += 2;
              y1 = k1->ys + 3;
            } else {
  ...
  if (m2 >= 0) {
    ...
    if (k2->abc_type == ABC_T_NOTE) {
      if (s > 0) {
        if (k2->stem > 0) {
          x2 += 1;
          if ((k2->sflags & S_BEAM_ST) && k2->nflags >= -1 &&
...
static void draw_slurs(struct SYMBOL *first, struct SYMBOL *last) {
  ...
  for (;;) {
    ...
    if ((s->type != NOTEREST && s->type != SPACE) ||
        (s->u.note.slur_st == 0 && !(s->sflags & S_SL1))) {
      s = s->next;
      continue;
    }
    ...
    for (;;) {
      ...
      if (s1->type == BAR &&
          ((s1->sflags & S_RRBAR) || s1->u.bar.type == B_THIN_THICK ||
           s1->u.bar.type == B_THICK_THIN ||
           (s1->u.bar.repeat_bar && s1->text && s1->text[0] != '1'))) {
        k = s1;
        break;
      }
      ...
      if (s1->u.note.slur_end || (s1->sflags & S_SL2)) {
        k = s1;
        break;
      }
      if (s1->u.note.slur_st || (s1->sflags & S_SL1)) {
    ...
    if ((k->type == NOTEREST || k->type == SPACE) &&
        (k->u.note.slur_end || (k->sflags & S_SL2))) {
      ...
      if (k->type != BAR ||
          (!(k->sflags & S_RRBAR) && k->u.bar.type != B_THIN_THICK &&
           k->u.bar.type != B_THICK_THIN &&
           (!k->u.bar.repeat_bar || !k->text || k->text[0] == '1')))
        cont = 1;
    }
    ...
    if (s->u.note.slur_st || (s->sflags & S_SL1))
      continue;
    if (s == last)
      break;
    s = s->next;
                                  ...
                                  struct SYMBOL *s) /* main note */
{
  ...
  for (s2 = s; s2; s2 = s2->next) {
    ...
    if (s2->type != NOTEREST) {
      if (s2->type == GRACE) {
        for (g = s2->extra; g; g = g->next) {
          if (g->type != NOTEREST)
            continue;
          if (g->u.note.slur_st || (g->sflags & S_SL1))
            some_slur = 1;
        }
      }
      continue;
    ...
    if (s2->u.note.slur_st /* if slur start/end */
        || s2->u.note.slur_end || (s2->sflags & (S_SL1 | S_SL2)))
      some_slur = 1;
    if (s2->staff < upstaff)
      upstaff = s2->staff;
    if (!s1)
  ...
  if (s1 == s2) { /* tuplet with 1 note (!) */
    ...
    if ((t->aux & 0xf000) == 0x2000 /* if 'when' == always */
      ...
      for (sy = s1;; sy = sy->next) {
        if (sy->type != NOTEREST) {
          if (sy->type == GRACE || sy->type == SPACE)
            continue;
          nb_only = 0;
          break;
        }
      ...
      if (nb_only && !(s1->sflags & (S_BEAM_ST | S_BEAM_BR1 | S_BEAM_BR2))) {
    ...
    if (s2->dur > s2->prev->dur) {
      ...
      if (!sy // maybe a note in an overlay voice
        ...
        for (sy = s2->ts_next; sy; sy = sy->ts_next) {
          if ((sy->sflags & S_SEQST) && sy->time >= s2->time + s2->dur)
            break;
        }
      }
      x2 = sy ? sy->x - sy->wl - 5 : state.realwidth - 6;
    } else {
      x2 = s2->x + 2;
...
static void draw_note_ties(struct SYMBOL *k1, struct SYMBOL *k2, int ntie,
                           unsigned char *mhead1, unsigned char *mhead2,
                           int job) {
...
//			}
    ...
    h = (.04 * (x2 - x1) + 10) * s;
    h *= cfmt.tieheight;
    slur_out(x1, staff_tb[staff].y + y, x2, staff_tb[staff].y + y, s, h,
             k1->u.note.notes[m1].ti1 & SL_DOTTED, -1);
  }
}

/* -- draw ties between neighboring notes/chords -- */
static void draw_ties(struct SYMBOL *k1, struct SYMBOL *k2,
                      int job) /* 0: normal
                                * 1: no starting note
                                * 2: no ending note
                                * 3: no start for clef or staff change */
{
  ...
  while (k3 && k3->time < time)
    k3 = k3->ts_next;
  while (k3 && k3->time == time) {
    if (k3->abc_type != ABC_T_NOTE || k3->staff != k1->staff) {
      k3 = k3->ts_next;
      continue;
    }
    ntie = 0;
    ...
    if (ntie > 0) {
      draw_note_ties(k1, k3, ntie, mhead1, mhead2, job == 1 ? 1 : 0);
      if (ntie3 == 0)
        return;
    }
    k3 = k3->ts_next;
...
static void draw_ties_g(struct SYMBOL *s1, struct SYMBOL *s2, int job) {
  struct SYMBOL *g;
...
static void draw_all_ties(struct VOICE_S *p_voice) {
  ...
  for (s2 = s1; s2; s2 = s2->next) {
    ...
    if (s2->dur || s2->type == GRACE)
      break;
    if (s2->type != BAR || !s2->u.bar.repeat_bar || !s2->text)
      continue;
  ...
  for (;;) {
    for (s1 = s2; s1; s1 = s1->next) {
      ...
      if (!rtie)
        continue;
      if (s1->type != BAR || !s1->u.bar.repeat_bar || !s1->text)
        continue;
    ...
    if (!s2) {
      ...
      if (s2->abc_type != ABC_T_NOTE && s2->abc_type != ABC_T_BAR) {
        error(1, s1, "Bad tie");
        continue;
      }
      if (s2->time == time) {
...
static void draw_all_slurs(struct VOICE_S *p_voice) {
  ...
  for (; s; s = s->next) {
    if (s->type != NOTEREST && s->type != SPACE)
      continue;
    while (s->u.note.slur_end || (s->sflags & S_SL2)) {
...
static void draw_tblt_w(struct VOICE_S *p_voice, int nly, float y,
                        struct tblt_s *tblt) {
  struct SYMBOL *s;
  struct lyrics *ly;
  struct lyl *lyl;
  ...
  set_font(VOCALFONT);
  a2b("%.1f 0 y %d %s\n", state.realwidth, nly, tblt->head);
  for (j = 0; j < nly; j++) {
    for (s = p_voice->sym; s; s = s->next) {
      ly = s->ly;
      if (!ly || (lyl = ly->lyl[j]) == NULL) {
        if (s->type == BAR) {
          if (!tblt->bar)
            continue;
          p = &tex_buf[16];
...
static void draw_tblt_p(struct VOICE_S *p_voice, float y, struct tblt_s *tblt) {
  struct SYMBOL *s;
  ...
  for (s = p_voice->sym; s; s = s->next) {
    ...
    if (acc != 0) {
      workmap[pitch] = acc == A_NT ? A_NULL : (acc & 0x07);
    }
    pitch = scale[pitch % 7] + acc_pitch[workmap[pitch]] + 12 * (pitch / 7) -
            tblt->pitch;
    octave = 0;
    while (pitch < 0) {
      pitch += 12;
...
static void draw_lyric_line(struct VOICE_S *p_voice, int j) {
  struct SYMBOL *s;
  struct lyrics *ly;
  struct lyl *lyl;
  int hyflag, l, lflag;
  ...
  for (s = p_voice->sym; /*s*/; s = s->next)
    if (s->type != CLEF && s->type != KEYSIG && s->type != TIMESIG)
      break;
  if (s->prev)
    lastx = s->prev->x;
  else
  ...
  for (; s; s = s->next) {
    if (s->abc_type == ABC_T_NOTE) {
      if (s->ly && s->ly->lyl[j] && s->ly->lyl[j]->t[0] == LY_UNDER) {
        lflag = 1;
        x0 = state.realwidth - 15;
        if (x0 < lastx + 12)
          x0 = lastx + 12;
      }
      break;
...
static float draw_lyrics(struct VOICE_S *p_voice, int nly, float *h, float y,
                         int incr) /* 1: below, -1: above */
{
  ...
  if (p_voice->tblts[0]) {
    if (p_voice->tblts[0]->pitch == 0)
      return y; /* yes */
    if (p_voice->tblts[1] && p_voice->tblts[1]->pitch == 0)
      return y; /* yes */
  }

  str_font(VOCALFONT);
  state.outft = -1; /* force font output */
...
static void draw_all_lyrics(void) {
  ...
  for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
    if (p_voice->have_ly || p_voice->tblts[0])
      break;
  }
  if (!p_voice)
    return;
  ...
  for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
      ...
      for (s = p_voice->sym; s; s = s->next) {
        ...
        for (i = 0; i < MAXLY; i++) {
          ...
          if (lyvo_tb[voice].h[i] < ly->lyl[i]->f->size)
            lyvo_tb[voice].h[i] = ly->lyl[i]->f->size;
        }
      }
    } else {
      y = y_get(p_voice->staff, 1, 0, state.realwidth);
      if (top < y)
        top = y;
      y = y_get(p_voice->staff, 0, 0, state.realwidth);
      if (bot > y)
        bot = y;
    }
    ...
    if (p_voice->posit.voc != 0)
      above_tb[voice] = p_voice->posit.voc == SL_ABOVE;
             ...
             && p_voice->next->staff == staff && p_voice->next->have_ly)
      above_tb[voice] = 1;
    if (above_tb[voice])
      lyst_tb[staff].a = 1;
    else
  ...
  for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
    ...
    if (lyvo_tb[voice].nly > 0)
      lyst_tb[staff].bot = draw_lyrics(p_voice, lyvo_tb[voice].nly,
                                       lyvo_tb[voice].h, lyst_tb[staff].bot, 1);
    sc = staff_tb[p_voice->staff].staffscale;
    for (nly = 0; nly < 2; nly++) {
      ...
      if (tblt->pitch == 0)
        draw_tblt_w(p_voice, lyvo_tb[voice].nly,
                    lyst_tb[staff].bot * sc - tblt->hu, tblt);
      else
        draw_tblt_p(p_voice,
                    //					lyst_tb[staff].bot,
                    lyst_tb[staff].bot * sc - tblt->hu, tblt);
  ...
  while (--i >= 0) {
    ...
    lyst_tb[staff].top = draw_lyrics(p_voice, lyvo_tb[voice].nly,
                                     lyvo_tb[voice].h, lyst_tb[staff].top, -1);
  }
  ...
  for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
    ...
    if (lyst_tb[staff].b) {
      ...
      if (lyvo_tb[p_voice - voice_tb].nly > 0) {
        ...
        y_set(staff, 0, 0, state.realwidth, bot);
      }
    }
  }
}
...
void draw_sym_near(void) {
  ...
  for (s = tsfirst; s; s = s->ts_next) {
    ...
    if (s->type == GRACE) {
      for (g = s->extra; g; g = g->next) {
        y_set(s->staff, 1, g->x - 2, 4, g->ymx + 1);
        y_set(s->staff, 0, g->x - 2, 4, g->ymn - 1);
      }
      continue;
    }
    if (s->type != MREST) {
...
static void draw_vname(float indent) {
  ...
  for (staff = nstaff; staff >= 0; staff--) {
    ...
    if (staff_p->nl == 0)
      continue;
    y = staff_tb[staff].y +
        staff_tb[staff].topbar * .5 * staff_tb[staff].staffscale +
        9 * (staff_p->nl - 1) - cfmt.font_tb[VOICEFONT].size * .3;
    n = staff;
...
static float set_staff(void) {
  ...
  staffsep =
      cfmt.staffsep * 0.5 + staff_tb[staff].topbar * staff_tb[staff].staffscale;
  if (y < staffsep)
    y = staffsep;
  staff_tb[staff].y = -y;
  ...
  for (staff++; staff <= nstaff; staff++) {
    ...
    dy = 0;
    if (staff_tb[staff].staffscale == staff_tb[prev_staff].staffscale) {
      for (i = 0; i < YSTEP; i++) {
        v = staff_tb[staff].top[i] - staff_tb[prev_staff].bot[i];
        if (dy < v)
          dy = v;
      }
      ...
      for (i = 0; i < YSTEP; i++) {
        v = staff_tb[staff].top[i] * staff_tb[staff].staffscale -
            staff_tb[prev_staff].bot[i] * staff_tb[prev_staff].staffscale;
        if (dy < v)
          dy = v;
      }
    }
    staffsep += staff_tb[staff].topbar * staff_tb[staff].staffscale;
    if (dy < staffsep)
      dy = staffsep;
    maxsep += staff_tb[staff].topbar * staff_tb[staff].staffscale;
    if (dy > maxsep)
      dy = maxsep;
    y += dy;
    staff_tb[staff].y = -y;
  ...
  for (staff = nstaff; staff >= 0; staff--) {
    dy = staff_tb[staff].y;
    if (staff_tb[staff].staffscale != 1 && staff_tb[staff].staffscale != 0) {
...
float draw_systems(float indent) {
  ...
  for (staff = 0; staff <= nstaff; staff++) {
    ...
    if ((x = xstaff[staff]) < 0)
      continue;
    draw_staff(staff, x, state.realwidth);
  }
  set_sscale(-1);
  return line_height;
}
...
static void draw_symbols(struct VOICE_S *p_voice) {
  ...
  for (s = p_voice->sym; s; s = s->next) {
    ...
    if (s->flags & ABC_F_INVIS) {
      switch (s->type) {
      case KEYSIG:
        memcpy(&p_voice->key, &s->u.key, sizeof p_voice->key);
      default:
        continue;
      case NOTEREST:
      case GRACE:
    ...
    case NOTEREST:
      ...
      if (s->abc_type == ABC_T_NOTE) {
        ...
        if (s == bm.s2)
          bm.s2 = NULL;
        if (options.annotate &&
            (s->sflags & (S_BEAM_ST | S_BEAM_END)) == S_BEAM_END)
          anno_out(s, 'e');
        break;
      }
      draw_rest(s);
    ...
    case CLEF:
      ...
      if ((s->flags & ABC_F_INVIS) || staff_tb[staff].empty)
        break;
      set_color(0);
      set_sscale(staff);
      y = staff_tb[staff].y;
      ...
      if (s->u.clef.name)
        ...
        a2b("%c%cclef\n", s->aux ? 's' : ' ', "tcbp"[(unsigned)s->u.clef.type]);
    ...
    case TIMESIG:
      ...
      memcpy(&p_voice->meter, &s->u.meter, sizeof p_voice->meter);
      if ((s->sflags & S_SECOND) || staff_tb[s->staff].empty)
        break;
    ...
    case MREST:
      ...
      a2b("mrest(%d)/Times-Bold 15 selectfont ", s->u.bar.len);
...
void draw_all_symb(void) {
  ...
  for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
    ...
    if (p_voice->sym)
#else
    if (p_voice->sym && !staff_tb[p_voice->staff].empty)
#endif
...
void putf(float v) { a2b("%.1f ", v); }

void putx(float x) { putf(x / cur_scale); }

void puty(float y) {
  putf(scale_voice ? y / cur_scale : /* scaled voice */
           y - cur_trans);           /* scaled staff */
}

void putxy(float x, float y) {
  if (scale_voice)
    a2b("%.1f %.1f ", x / cur_scale, y / cur_scale); /* scaled voice */
  else
    a2b("%.1f %.1f ", x / cur_scale, y - cur_trans); /* scaled staff */
}
  ...
  for (s = sym; s; s = s->next) {
    ...
    for (i = 0; i <= s->nhd; i++) {
      if (s->u.note.notes[i].ti1) {
        ntie++;
        if (pit < 128 && s->u.note.notes[i].pit <= pit + 1)
          sec++;
    ...
    if (sec == 0) {
      if (ntie & 1) {
        /* in chords with an odd number of notes, the outer noteheads are paired
         * off center notes are tied according to their position in relation to
         * the center line */
        ntie /= 2;
        dir = SL_BELOW;
        for (i = 0; i <= s->nhd; i++) {
...
static void set_tie_room(void) {
  ...
  for (p_voice = first_voice; p_voice; p_voice = p_voice->next) {
    ...
    for (; s; s = s->next) {
      ...
      if (!(s->sflags & S_TI1))
        continue;
      if (s->pits[0] < 20 && (s->u.note.notes[0].ti1 & 0x07) == SL_BELOW)
        ;
