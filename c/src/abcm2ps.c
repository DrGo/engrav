...
struct state_s state={.pagenum = 1,.pagenum_nr = 1,
/* width of notes indexed by log2(note_length) */
.space_tb={ 7, 10, 14.15, 20, 28.3, 40,/* crotchet */ 56.6, 80, 113, 150
		...
		char *rfn)	/* returned real file name */
{
	...
	if (options.in_fname && options.in_fname != fn
	 && (p = strrchr(options.in_fname, DIRSEP)) != NULL) {
		l = p - options.in_fname + 1;
		strncpy(rfn, options.in_fname, l);
		strcpy(&rfn[l], fn);
...
static void treat_file(char *fn, char *ext)
{
	...
	if (!options.quiet)
		fprintf(strcmp(options.outfn, "-") == 0 ? stderr : stdout,
			"File %s\n", abc_fn);
...
#endif
		if (l2 <= 0) {
			fwrite(p, 1, l, options.fout);
			break;
		}
		q++;
		fwrite(p, 1, q - p, options.fout);
		l -= q - p;
		while (p != q) {
			if (*p++ == '\n')
				linenum++;
...
//			break;
		...
		clrarena(1);			/* clear previous tunes */
		options.file_initialized = -1;	/* don't put <br/> before first image */
...
{
	FILE *log = strcmp(options.outfn, "-") == 0 ? stderr : stdout;
	...
	while (--argc > 0) {
		...
		while ((c = *++p) != '\0') {	/* '-xxx' */
			...
			case 'E':
				...
				break;
			case 'g':
				options.svg = 0;	/* SVG one file per tune */
				options.epsf = 2;
				break;
			case 'H':
				options.quiet = 1;	// don't output the version
				break;
			case 'h':
				usage();	/* no return */
			case 'p':
				options.pipeformat = 1;	/* format for bagpipe regardless of key */
				break;
			case 'q':
				options.quiet = 1;
				break;
			...
			case 'V':
				...
				return EXIT_SUCCESS;
			case 'v':
				options.svg = 1;	/* SVG one file per page */
				options.epsf = 0;
				break;
			case 'X':
				options.svg = 2;	/* SVG/XHTML */
				options.epsf = 0;
				break;
			case 'k': {
				int kbsz;
			...
			case 'O':
				...
				if (strlen(aaa) >= sizeof options.outfn) {
					error(1, NULL, "'-O' too large - aborting");
					return EXIT_FAILURE;
				}
				strcpy(options.outfn, aaa);
				break;
			case 'z':
				options.epsf = 3;	/* ABC embedded in XML */
				options.svg = 0;
				break;
			default:
...
//	memset(&info, 0, sizeof info);
	...
	while (--argc > 0) {
		...
		if (c == '-') {
			...
			if (p[1] == '\0') {		/* '-' alone */
				if (options.in_fname) {
					treat_abc_file(options.in_fname);
					frontend((unsigned char *) "select\n", FE_FMT,
							"cmd_line", 0);
				}
				options.in_fname = "";		/* read from stdin */
				continue;
			}
			i = strlen(p) - 1;
			if (p[i] == '-'
...
//					 def_fmt_done = 1;
			...
			while ((c = *++p) != '\0') {
					...
					if (p[1] == '\0'
					 && (argc <= 1
					  || !isdigit((unsigned) argv[1][0]))) {
						options.pagenumbers = 2; /* old behaviour */
						break;
					}
					/* fall thru */
				...
				case 'w':
					...
					case 'm':
						...
						break;
					case 'N':
						sscanf(aaa, "%d", &options.pagenumbers);
						if ((unsigned) options.pagenumbers > 4) {
							error(1, NULL,
	...
	if (!options.epsf && !options.fout) {
		error(1, NULL, "Nothing to generate!");
		return EXIT_FAILURE;
	}
	close_output_file();
	return state.severity == 0 ? EXIT_SUCCESS : EXIT_FAILURE;
}
