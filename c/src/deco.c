		...
		float w)
{
	...
	i = (int) (x / state.realwidth * YSTEP);
	if (i < 0) {
//		fprintf(stderr, "y_get i:%d\n", i);
		i = 0;
	}
	j = (int) ((x + w) / state.realwidth * YSTEP);
	if (j >= YSTEP) {
		j = YSTEP - 1;
		if (i > j)
			i = j;
...
static void d_gliss(struct deco_elt *de2)
{
	...
	de2->x -= 2 + s2->u.note.notes[0].shac ?
			(s2->u.note.notes[0].shac + 3) : state.hw_tb[s2->head];
}
		...
		if (dd->strx != 0) {
			...
			if (dd->strx == 255) {
				...
				if (*p == '@') {
					...
					state.outft = -1;	// force font selection
					putxy(x, y);
					a2b("M");
					put_str(p, A_LEFT);
					continue;
