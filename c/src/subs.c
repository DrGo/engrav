...
static void str_ft_out(char *p, int end)
{
	...
	use_glyph = !options.svg && options.epsf <= 1 &&	/* not SVG */
			...
			if (isdigit((unsigned char) p[1])
			 && (unsigned) (p[1] - '0') < FONT_UMAX) {
				...
				if (curft != p[1] - '0') {
					...
					if (curft == 0)
						curft = defft;
					use_glyph = !options.svg && options.epsf <= 1 &&
...
//			stropx++;
	...
	if (options.svg || options.epsf > 1)		/* not for SVG */
		return;
