...
{
	if (enc == 0)		/* utf-8 */
		fprintf(options.fout, "/%s-utf8/%s mkfont\n"
			"/F%d{/%s-utf8 exch selectfont}!\n",
			name, name, num, name);
	else			/* native encoding */
		fprintf(options.fout, "/F%d{/%s exch selectfont}!\n", num, name);
}
...
void define_symbols(void)
{
	...
	p = cfmt.musicfont;
	fputs(ps_head, options.fout);
	fputs(p ? psfgl : psdgl, options.fout);
	...
	if (p) {
		...
		if (q) {		// hope "url(...)"
			...
			if (p)
				q = p + 1;
			fprintf(options.fout, "/music/%.*s def\n", (int) (r - q), q);
		} else {
			fprintf(options.fout, "/music/%s def\n", p);
		}
	}
	...
	/* len su - up stem */
	fprintf(options.fout, "/su{dlw x y M %.1f %.1f RM %.1f sub 0 exch RL stroke}!\n",
		STEM_XOFF, STEM_YOFF, STEM_YOFF);

	/* len sd - down stem */
	fprintf(options.fout, "/sd{dlw x y M %.1f %.1f RM %.1f add 0 exch RL stroke}!\n",
		-STEM_XOFF, -STEM_YOFF, STEM_YOFF);

	/* n len sfu - stem and n flags up */
	fprintf(options.fout, "/sfu{	dlw x y M %.1f %.1f RM\n"
	...
	fprintf(options.fout, "/sfd{	dlw x y M %.1f %.1f RM\n"
	...
	fprintf(options.fout, "/sfs{	dup 0 lt{\n"
		"		dlw x y M -%.1f -%.1f RM\n"
		"		%.1f add 0 exch RL currentpoint stroke\n"
		"		M{	currentpoint\n"
		"			7 %.1f RL\n"
	...
	fprintf(options.fout, "/gu{	.6 SLW x y M\n"
	...
	fprintf(options.fout, "/sgu{	.6 SLW x y M %.1f 0 RM\n"
	...
	fprintf(options.fout, "/sgd{	.6 SLW x y M %.1f 0 RM\n"
		"	0 exch RL currentpoint stroke\n"
		"	M dup 1 eq{\n"
		"		pop\n"
		"		0.6 3.4 5.6 3.8 3 10 RC\n"
	...
	fprintf(options.fout, "/sgs{	.6 SLW x y M %.1f 0 RM\n"
		"	0 exch RL currentpoint stroke\n"
		"	M{	currentpoint\n"
