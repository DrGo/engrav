...
static int get_font(const char *fname, int encoding) {
  ...
  while (--fnum >= 0) {
    if (strcmp(fname, fontnames[fnum]) == 0 && encoding == font_enc[fnum])
      return fnum;
  }
  ...
  if (options.epsf <= 1 && !options.svg) {
    if (options.file_initialized > 0)
      error(1, NULL, "Cannot have a new font when the output file is opened");
    if (strchr(fname, ' ') != NULL) {
      error(1, NULL, "PostScript fonts cannot have names with spaces");
      return 0;
    }
  }
  fnum = nfontnames++;
...
static int dfont_set(struct FONTSPEC *f) {
  ...
  for (i = FONT_DYN; i < cfmt.ndfont; i++) {
    if (cfmt.font_tb[i].fnum == f->fnum && cfmt.font_tb[i].size == f->size)
      return i;
  }
  if (i >= FONT_MAX - 1) {
...
/* -- define a font -- */
static void fontspec(struct FONTSPEC *f, const char *name, int encoding,
                     float size) {
  if (name)
  ...
  } else if (strncmp(name, times, 5) == 0 || strncmp(name, serif, 5) == 0) {
    if (strcmp(name, times_bold) == 0 || strcmp(name, serif_bold) == 0)
      f->swfac *= 1.05;
...
void define_fonts(void) {
  ...
  static char *mkfont =
      ...
      "		CharStrings/sharp{pop .46 0 setcharwidth .001 dup scale usharp "
      "ufill}bind put\n"
      "		CharStrings/flat{pop .46 0 setcharwidth .001 dup scale uflat "
      "ufill}bind put\n"
      "		CharStrings/natural{pop .40 0 setcharwidth .001 dup scale unat "
      "ufill}bind put\n"
      "		CharStrings/double_sharp{pop .46 0 setcharwidth .001 dup scale "
      "udblesharp ufill}bind put\n"
      "		CharStrings/double_flat{pop .50 0 setcharwidth .001 dup scale "
      "udbleflat ufill}bind put\n"
      "	    }{\n"
      "		CharStrings/sharp{pop 460 0 setcharwidth usharp ufill}bind "
      "put\n"
      "		CharStrings/flat{pop 460 0 setcharwidth uflat ufill}bind put\n"
      "		CharStrings/natural{pop 400 0 setcharwidth unat ufill}bind "
      "put\n"
      "		CharStrings/double_sharp{pop 460 0 setcharwidth udblesharp "
      "ufill}bind put\n"
      "		CharStrings/double_flat{pop 500 0 setcharwidth udbleflat "
      "ufill}bind put\n"
      "	    }ifelse\n"
      "	}if currentdict definefont pop end}!\n";

  fputs(mkfont, options.fout);
  make_font_list();
...
void set_format(void) {
  ...
  f->textoption = T_LEFT;
  f->ndfont = FONT_DYN;
  if (options.svg || options.epsf > 2) { // SVG output
    ...
    fontspec(&f->font_tb[INFOFONT], serif_italic, 0,
             14.0); /* same as composer by default */
    ...
    fontspec(&f->font_tb[INFOFONT], times_italic, 0,
             14.0); /* same as composer by default */
...
void print_format(void) {
  ...
  for (fd = format_tb; fd->name; fd++) {
    ...
    case FORMAT_I:
      ...
      case 3: /* tuplets */
        printf("%d %d %d %d\n", cfmt.tuplets >> 12, (cfmt.tuplets >> 8) & 0x0f,
               (cfmt.tuplets >> 4) & 0x0f, cfmt.tuplets & 0x0f);
        break;
        //			case 4:			/* textoption */
        //				break;
      case 5: /* gracespace */
        printf("%d.%d %d.%d %d.%d\n", (cfmt.gracespace >> 16) / 10,
               ...
               ((cfmt.gracespace >> 8) & 0xff) % 10,
               (cfmt.gracespace & 0xff) / 10, (cfmt.gracespace & 0xff) % 10);
        break;
      }
      break;
    case FORMAT_R:
    ...
    case FORMAT_U:
      if (fd->subtype == 0)
        ...
        printf("%.2fcm\n",
               (cfmt.pagewidth - cfmt.leftmargin - cfmt.rightmargin) / (1 CM));
      break;
    case FORMAT_S:
...
/* -- get an encoding -- */
static int parse_encoding(char *p) { return strncasecmp(p, "native", 6) == 0; }

/* -- get a position -- */
static int get_posit(char *p) {
  if (strcmp(p, "up") == 0 || strcmp(p, "above") == 0)
    return SL_ABOVE;
  if (strcmp(p, "down") == 0 || strcmp(p, "below") == 0 ||
      strcmp(p, "under") == 0)
    return SL_BELOW;
  if (strcmp(p, "hidden") == 0 || strcmp(p, "opposite") == 0)
    return SL_HIDDEN;
  if (strcmp(p, "auto") == 0)
...
int get_textopt(char *p) {
  if (strncmp(p, "align", 5) == 0 || strncmp(p, "justify", 7) == 0)
    return T_JUSTIFY;
  if (strncmp(p, "ragged", 6) == 0 || strncmp(p, "fill", 4) == 0)
    return T_FILL;
...
static void g_fspc(char *p, struct FONTSPEC *f) {
  char fname[80];
  int encoding;
  float fsize;
  ...
  fontspec(f, strcmp(fname, "*") != 0 ? fname : NULL, encoding, fsize);
  if (options.file_initialized <= 0)
    used_font[f->fnum] = 1;
  if (f - cfmt.font_tb == state.outft)
    state.outft = -1;
#ifdef HAVE_PANGO
  pg_reset_font();
#endif
...
static const struct vpar vpar_tb[] = {{"dynamic", set_dyn},  /* 0 */
                                      {"gchord", set_gch},   /* 1 */
                                      {"gstemdir", set_gsd}, /* 2 */
                                      {"ornament", set_orn}, /* 3 */
                                      {"stemdir", set_std},  /* 4 */
...
void interpret_fmt_line(char *w, /* keyword */
  ...
  case 'f':
    if (strcmp(w, "font") == 0) {
      ...
      if (options.file_initialized > 0 && !options.svg &&
          options.epsf <= 1) { /* PS */
        error(1, NULL, "Cannot define a font when the output file is opened");
        return;
      }
      p = get_str(fname, p, sizeof fname);
      ...
      for (i = FONT_UMAX; i < FONT_MAX; i++) {
        if (cfmt.font_tb[i].fnum == fnum)
          cfmt.font_tb[i].swfac = cfmt.font_tb[i].size * swfac;
      }
      return;
    }
    break;
...
#ifdef HAVE_PANGO
    case 2:                                /* %%pango = 0, 1 or 2 */
      if (options.svg || options.epsf > 1) // if SVG output
        break;
      if (*p == '2') {
        cfmt.pango = 2;
        break;
  ...
  case FORMAT_I:
    ...
    if (fd->subtype == 5) { /* gracespace */
      ...
      if (sscanf(p, "%f %f %f", &f1, &f2, &f3) != 3 || f1 > 25 || f2 > 25 ||
          f3 > 25)
        goto bad;
      i1 = f1 * 10;
      i2 = f2 * 10;
    ...
    *((float *)fd->v) = scan_u(p, fd->subtype);
    ...
    case 1:
      if (strcmp(fd->name, "rightmargin") != 0 &&
        ...
        break;
      staffwidth = cfmt.pagewidth - cfmt.leftmargin - cfmt.rightmargin;
      if (staffwidth > 100)
        break;
      error(1, NULL, "'staffwidth' too small\n");
      staffwidth = 100;
      if (fd->name[0] == 'r')
        cfmt.rightmargin = cfmt.pagewidth - cfmt.leftmargin - staffwidth;
      else
        cfmt.leftmargin = cfmt.pagewidth - cfmt.rightmargin - staffwidth;
      break;
    case 2: /* staffwidth */
      if (staffwidth < 100) {
        error(1, NULL, "'staffwidth' too small\n");
      ...
      f = (cfmt.landscape ? cfmt.pageheight : cfmt.pagewidth) - staffwidth -
          cfmt.leftmargin;
      if (f < 0)
...
void set_font(int ft) {
  ...
  if (!used_font[fnum] && options.epsf <= 1 &&
      !options.svg) { /* (not usefull for svg output) */
    if (options.file_initialized <= 0) {
      used_font[fnum] = 1;
    } else {
      error(1, NULL, "Font '%s' not predefined; using first in list",
            fontnames[fnum]);
      fnum = 0;
    }
  }
  if (f->size == 0) {
    error(0, NULL, "Font '%s' with a null size - set to 8", fontnames[fnum]);
    f->size = 8;
  }
...
int get_font_encoding(int ft) { return font_enc[cfmt.font_tb[ft].fnum]; }
