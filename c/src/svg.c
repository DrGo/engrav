...
static void xml_str_out(char *p)
{
	...
	for (q = p; *p != '\0';) {
		...
		if (p - 1 != q)
			fwrite(q, 1, p - 1 - q, options.fout);
		q = p;
		fputs(r, options.fout);
	}
	if (p != q)
		fputs(q, options.fout);
}
...
#ifndef NoSVGDate
	fprintf(options.fout, "<!-- CreationDate: %s -->\n"
			"<!-- CommandLine:",
			tex_buf);
#else 
	fprintf(options.fout, "<!-- CommandLine:");
#endif 
...
static void define_head(float w, float h)
{
	...
	fprintf(options.fout, svg_head1, w, h);
	if (cfmt.musicfont) {
		if (strchr(cfmt.musicfont, '('))
			fprintf(options.fout, svg_font_style_url, cfmt.musicfont);
		else
			fprintf(options.fout, svg_font_style, cfmt.musicfont);
	}
	fputs(svg_head2, options.fout);
}
...
void define_svg_symbols(char *title, int num, float w, float h)
{
	...
	if (options.svg == 2) {			/* if XHTML */
		if (options.file_initialized <= 0) {
			if ((s = strrchr(options.in_fname, DIRSEP)) == NULL)
				s = options.in_fname;
			else
				s++;
			...
			if (cfmt.bgcolor && cfmt.bgcolor[0] != '\0')
				fprintf(options.fout, " background-color:%s",
						cfmt.bgcolor);
			fprintf(options.fout,
				"}\n"
				"\t@page {margin: 0}\n"
		...
		define_head(w, h);
		xml_str_out(title);
		fprintf(options.fout, svg_head3, "page", num);
//		if (cfmt.bgcolor && cfmt.bgcolor[0] != '\0')
//			fprintf(options.fout,
...
//				fputs("<p>\n", fout);
		...
		define_head(w, h);
		xml_str_out(title);
		fprintf(options.fout, svg_head3, options.epsf ? "tune" : "page", num);
...
static void output_font(int span)
{
	...
	fprintf(options.fout, " style=\"");

	if (!span && gcur.rgb) {
		fprintf(options.fout, "color:#%06x;", gcur.rgb);
		if (gcur.font_n[0] == '\0') {
			fprintf(options.fout, "\"");
			return;
		}
	...
	p = strstr(fn, "blique");
	if (p && (p[-1] == 'O' || p[-1] == 'o')) {
		fprintf(options.fout, "oblique ");
		i = p - fn - 1;
		if (imin > i)
			imin = i;
	}
	if (strchr(fn, ' '))
		fprintf(options.fout, "%.2fpx '%.*s'\"", gcur.font_s, imin, fn);
	else
		fprintf(options.fout, "%.2fpx %.*s\"", gcur.font_s, imin, fn);
}
...
{
	setg(0);
	fprintf(options.fout, "<g stroke-width=\"%.2f\"", gcur.linewidth);
	if (gcur.xscale != 1 || gcur.yscale != 1 || gcur.rotate != 0) {
		fprintf(options.fout, " transform=\"");
		if (gcur.xscale != 1 || gcur.yscale != 1) {
			if (gcur.xscale == gcur.yscale)
				fprintf(options.fout, "scale(%.3f)", gcur.xscale);
			else
				fprintf(options.fout, "scale(%.3f,%.3f)",
		...
		if (gcur.rotate != 0) {
			if (gcur.xoffs != 0 || gcur.yoffs != 0) {
				...
				x = xtmp * _cos - y * _sin;
				y = xtmp * _sin + y * _cos;
				fprintf(options.fout, " translate(%.2f, %.2f)", x, y);
				x_rot = gcur.xoffs;
				y_rot = gcur.yoffs;
				gcur.xoffs = 0;
				gcur.yoffs = 0;
			}
			fprintf(options.fout, " rotate(%.2f)", gcur.rotate);
		}
		fputs("\"", options.fout);
	}
	output_font(0);
...
//	}
	...
	while (i != 0 && !def_tb[i].defined) {
		def_tb[i].defined = 1;
		fputs(def_tb[i].def, options.fout);
		i = def_tb[i].use;
	}
	fputs(def_tb[def].def, options.fout);
	fputs("</defs>\n", options.fout);
}
...
static void xysym(char *op, int use)
{
	...
	fprintf(options.fout, "<use x=\"%.2f\" y=\"%.2f\" xlink:href=\"#%s\"/>\n",
		x, y, op);
}
...
static void arp_ltr(char type)
{
	...
	n = (pop_free_val() + 5) / 6;
	if (type == 'a') {
		fprintf(options.fout, "<g transform=\"rotate(270)\">\n");
		t = x;
		x = -y;
		y = t;
	}
	y -= 4;
	while (--n >= 0) {
		fprintf(options.fout, "<use x=\"%.2f\" y=\"%.2f\" xlink:href=\"#ltr\"/>\n",
			x, y);
		x += 6;
	}
	if (type == 'a')
		fprintf(options.fout, "</g>\n");
}
...
static void gliss(int squiggle)
{
	...
	if (squiggle) {
		...
		x1 = 8;
		while (--n >= 0) {
			fprintf(options.fout, "<use x=\"%.2f\" xlink:href=\"#ltr\"/>\n", x1);
			x1 += 6;
		}
	} else {
		fprintf(options.fout, "<path class=\"stroke\" stroke-width=\"1\"\n"
			"	d=\"M8 0l%.2f 0\"/>\n", len);
	}
	fprintf(options.fout, "</g>\n");
}
...
static void show(char type)
{
	...
	if (span) {
		fprintf(options.fout, "<tspan\n\t");
		output_font(1);
		fprintf(options.fout, ">");
	} else if (g != 2) {
		fprintf(options.fout, "<text x=\"%.2f\" y=\"%.2f\"",
				gcur.xoffs + x, gcur.yoffs - y);
		switch (type) {
		case 'c':
			fprintf(options.fout, " text-anchor=\"middle\"");
			w /= 2;
			break;
		case 'r':
			fprintf(options.fout, " text-anchor=\"end\"");
			w = 0;
			break;
		case 'j':
			fprintf(options.fout, " textLength=\"%.2f\"", w);
			break;
		}

//		if (gcur.rgb != 0)
//			fprintf(options.fout, " class=\"fill\"");
		fputs(">", options.fout);
		g = 2;
	}
...
back:
	...
	if (type == 'x') {
		...
		fprintf(options.fout, "<tspan dx=\"%.2f\">", w);
		span = 1;
		goto back;
	}
	if (type == 'b') {
...
//				path_def();
		...
		if (strcmp(op, "crdc") == 0) {
			...
			fprintf(options.fout, "<text style=\"font:italic 16px serif\"\n"
		...
		if (strcmp(op, "cresc") == 0) {
			...
			x += w;
			if ((int) sym->e->u.v & 1)
				fprintf(options.fout, "<path class=\"stroke\"\n"
	...
	case 'd':
		if (strcmp(op, "dacs") == 0) {
			...
			fprintf(options.fout, "<text style=\"font:16px serif\"\n"
				"	x=\"%.2f\" y=\"%.2f\" text-anchor=\"middle\">%s</text>\n",
				x, y, s + 1);
			free(s);
			return;
	...
	case 'e':
		...
		if (strcmp(op, "eofill") == 0) {
			...
			path_end();
			fprintf(options.fout, "\t\" fill-rule=\"evenodd\" class=\"fill\"/>\n");
			return;
		}
		if (strcmp(op, "eq") == 0) {
			cond(C_EQ);
...
//				ps_error = 1;
		...
		if (strcmp(op, "fng") == 0) {
			...
			fprintf(options.fout, "<text style=\"font:8px Bookman\"\n"
				"	x=\"%.2f\" y=\"%.2f\">%s</text>\n",
				x, y, s + 1);
			free(s);
			return;
...
//			setg(1);
	...
	case 'h':
		...
		if (strcmp(op, "hyph") == 0) {
			...
			n = (w - 15.) / d;
			x += (w - d * n - 5) / 2;
			fprintf(options.fout, "<path class=\"stroke\" stroke-width=\"1.2\"\n"
...
moveto:
	...
	case 'm':
		...
		if (strcmp(op, "mrest") == 0) {
			...
			fprintf(options.fout, "<use x=\"%.2f\" y=\"%.2f\" xlink:href=\"#mrest\"/>\n"
...
//			path_def();
	...
	case 'o':
		...
		if (strcmp(op, "oct") == 0) {
			...
			y = gcur.yoffs - pop_free_val();
			x = gcur.xoffs + pop_free_val();
			fprintf(options.fout, "<text style=\"font:12px serif\"\n"
				"	x=\"%.2f\" y=\"%.2f\">8</text>\n",
				x, y);
			return;
		}
	...
	case 'p':
		...
		if (strcmp(op, "pf") == 0) {
			...
			fprintf(options.fout, "<text style=\"font:bold italic 16px serif\"\n"
				"	x=\"%.2f\" y=\"%.2f\">%s</text>\n",
				x, y, s + 1);
			free(s);
			return;
	...
	case 'r':
		...
		if (strcmp(op, "repbra") == 0) {
			...
			fprintf(options.fout,
				...
				x, y);
			if (i & 1)
				fprintf(options.fout, "m0 20v-20");
			fprintf(options.fout, "h%.2f", w);
			if (i & 2)
				fprintf(options.fout, "v20");
			fprintf(options.fout, "\"/>\n");
			free(s);
			return;
		}
...
//				ps_error = 1;
			...
			path_end();
			fprintf(options.fout, "\t\" class=\"stroke\"%s/>\n",
					gcur.dash);
			return;
		}
		if (strcmp(op, "su") == 0
		...
		if (strcmp(op, "stsig") == 0) {
			...
			fprintf(options.fout, "<g style=\"font:bold 18px serif\"\n"
...
//			setg(1);
	...
	case 't':
		...
		if (strcmp(op, "trem") == 0) {
			...
			n = pop_free_val();
			fprintf(options.fout, "<path class=\"fill\" d=\"m%.2f %.2f\n\t",
				x, y);
			for (;;) {
				fputs("l9 -3v3l-9 3z", options.fout);
				if (--n <= 0)
					break;
				fputs("m0 5.4", options.fout);
			}
			fputs("\"/>", options.fout);
			return;
		}
		...
		if (strcmp(op, "tsig") == 0) {
			...
			fprintf(options.fout, "<g style=\"font:bold 16px serif\"\n"
				"	transform=\"translate(%.2f,%.2f) scale(1.2,1)\">\n"
	...
	case 'w':
		...
		if (strcmp(op, "wln") == 0) {
			...
			x = pop_free_val();
			w = pop_free_val();
			fprintf(options.fout, "<path class=\"stroke\" stroke-width=\"0.8\"\n"
				"	d=\"M%.2f %.2fh%.2f\"/>\n",
	...
	while (--len >= 0) {
		...
		case '%':
			...
			if (strncmp((char *) q, "A ", 2) == 0) {	/* annotation */
				...
				fprintf(options.fout, "<abc type=\"%c\" row=\"%d\" col=\"%d\" x=\"%.2f\" y=\"%.2f\" width=\"%.2f\" height=\"%d\"/>\n",
			...
			if (strncmp((char *) q, " --- title", 10) == 0) { /* title info */
				...
				if (q[10] == 's') {		/* subtitle */
					q += 14;
					fprintf(options.fout, "<!-- subtitle: %.*s -->\n",
							(int) (p - q - 1), q);
					break;
				}
				q += 11;
