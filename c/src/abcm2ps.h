...
#define BASE_LEN                                                               \
  1536 /* basic note length (semibreve or whole note - same as MIDI) */
...
struct FORMAT { /* struct for page layout */
  ...
  float gutter, indent, infospace, slurheight, tieheight, notespacingfactor,
      scale;
...
// SM: put all global options in a struct
struct options_s {
  ...
  int pipeformat;  /* format for bagpipes */
...
//SM putting all state vars together
struct state_s {
  int severity;
  ...
  int pagenum_nr; /* current page (non-resettable) */
...
#define C_XFLAGS 5           /* index of crotchet in flags tables */
  float space_tb[NFLAGS_SZ]; /* note spacing */
  float hw_tb[];             // width of note heads
};
...
/* extern struct map *maps; /1* note mappings *1/ */
...
void abc_eof(void);
char *get_str(char *d, char *s, int maxlen);
char *parse_acc_pit(char *p, int *pit, int *acc);
/* buffer.c */
void a2b(char *fmt, ...)
#ifdef __GNUC__
...
void write_tempo(struct SYMBOL *s, int beat, float sc);
float y_get(int staff, int up, float x, float w);
void y_set(int staff, int up, float x, float w, float y);
/* draw.c */
...
FILE *open_file(char *fn, char *ext, char *rfn);
...
void do_tune(void);
void identify_note(struct SYMBOL *s, int len, int *p_head, int *p_dots,
                   int *p_flags);
