...
static struct map *maps; /* note mappings */
...
static void sort_all(void) {
    ...
    for (r = 0; r < MAXVOICE; r++) {
      ...
      if (s->type == STAVES) { // change STAVES to a flag
        ...
        for (v2 = 0; v2 < MAXVOICE; v2++) {
          if (sy->next->voice[v2].range < 0 || sy->voice[v2].range >= 0)
            continue;
          vn[r2++] = v2;
        }
  ...
  if ((prev->type != BAR && prev->type != FMTCHG) || new_sy) {
  ...
  if (!parsys) {
    ...
    for (voice = 0; voice < MAXVOICE; voice++) {
      ...
      if (voice_tb[voice].stafflines)
        parsys->staff[staff].stafflines = voice_tb[voice].stafflines;
      if (voice_tb[voice].staffscale != 0)
        parsys->staff[staff].staffscale = voice_tb[voice].staffscale;
    }
...
static struct SYMBOL *go_global_time(struct SYMBOL *s,
  ...
  for (; s; s = s->ts_next) {
    if (s->type == BAR && s->aux >= symsel->bar)
      break;
  }
  if (!s)
    return NULL;
  ...
  if (clip_start.bar > 0 || clip_start.time > 0) {
    ...
    for (s2 = tsfirst; s2 != s; s2 = s2->ts_next) {
      ...
      case CLEF:
        ...
        break;
      case KEYSIG:
        memcpy(&voice_tb[s2->voice].key, &s2->u.key, sizeof voice_tb[0].key);
        break;
      case TIMESIG:
...
static void set_bar_num(void) {
  ...
  for (s = tsfirst;; s = s->ts_next) {
    ...
    case BAR:
      if (s->aux) {
        state.nbar = s->aux; /* (%%setbarnb) */
        break;
      }
      if (s->u.bar.repeat_bar && s->text && !cfmt.contbarnb) {
        if (s->text[0] == '1') {
          bar_rep = state.nbar;
        } else {
          state.nbar = bar_rep; /* restart bar numbering */
          s->aux = state.nbar;
        }
      }
      break;
  ...
  /* do the %%clip stuff */
  if (clip_start.bar >= 0) {
    if (bar_num <= clip_start.bar || state.nbar > clip_end.bar) {
      tsfirst = NULL;
      return;
    }
    do_clip();
  ...
  /* do the %%break stuff */
  {
    ...
    for (brk = brks; brk; brk = brk->next) {
      if (brk->symsel.bar <= nbar_min || brk->symsel.bar > bar_num)
        continue;
  ...
  if (cfmt.measurenb < 0) /* if no display of measure bar */
    state.nbar = bar_num;       /* update in case of more music to come */
}
...
static int acc_same_pitch(int pitch) {
  ...
  for (; s; s = s->prev) {
    ...
    case ABC_T_BAR:
      ...
      for (i = 0; i <= s->nhd; i++) {
        if (s->u.note.notes[i].pit == pitch && s->u.note.notes[i].ti1)
    ...
    if (a == 0) {
      if (curvoice->okey.nacc == 0) {
        ...
        for (j = 0; j < curvoice->okey.nacc; j++) {
          if ((n + 16 * 7 - curvoice->okey.pits[j]) % 7 == 0) {
...
static void gch_tr1(struct SYMBOL *s, int i, int i2) {
  char *p = &s->text[i], *q = p + 1, *new_txt;
  int l, latin;
  int n, a, i1, i3, i4;
  static const char note_names[] = "CDEFGAB";
  static const char *latin_names[7] = {"Do",  "Ré", "Mi", "Fa",
                                       "Sol", "La", "Si"};
...
static void gch_capo(struct SYMBOL *s) {
  ...
  static signed char cap_trans[] = {0, 5, -2, 3, -4, 1, -6, -1, 4, -3, 2, -5};
...
static struct SYMBOL *next_lyric_note(struct SYMBOL *s) {
  while (s && (s->abc_type != ABC_T_NOTE || (s->flags & ABC_F_GRACE)))
    s = s->next;
  return s;
}
  ...
  if (curvoice->ignore) {
    for (;;) {
      ...
      case ABC_T_PSCOM:
        ...
        continue;
      case ABC_T_INFO:
        if (s->abc_next->text[0] == 'w' || s->abc_next->text[0] == '+') {
          s = s->abc_next;
          continue;
        }
        break;
  ...
  for (;;) {
    ...
    while (*p != '\0') {
      ...
      case '|':
        ...
        if (!s1) {
          error(1, s2, "Not enough bar lines for lyric line");
          goto ly_next;
        }
        s2 = s1;
        s1 = s1->next;
      ...
      if (word[0] != '\0' && s1->posit.voc != SL_HIDDEN) {
        struct lyl *lyl;
        float w;
        ...
        lyl = (struct lyl *)getarena(sizeof *s1->ly->lyl[0] -
                                     sizeof s1->ly->lyl[0]->t + strlen(q) + 1);
        s1->ly->lyl[ln] = lyl;
        lyl->f = f;
        lyl->w = w;
        strcpy(lyl->t, q);
    ...
    for (;;) {
      ...
      case ABC_T_INFO:
        ...
        if (!cont) {
          s1 = next_lyric_note(s1);
          if (s1) {
            error(1, s1, "Not enough words for lyric line");
          }
        }
        break; /* more lyric */
      default:
...
static void get_over(struct SYMBOL *s) {
  ...
  if (p_voice->ignore)
    return;
  if (s->abc_type == ABC_T_BAR || s->u.v_over.type == V_OVER_E) {
    if (!p_voice->last_sym) {
  ...
  if (parsys->voice[voice2].range < 0) {
    ...
    memcpy(&p_voice2->key, &p_voice->key, sizeof p_voice2->key);
    memcpy(&p_voice2->ckey, &p_voice->ckey, sizeof p_voice2->ckey);
    memcpy(&p_voice2->okey, &p_voice->okey, sizeof p_voice2->okey);
    ...
    if (clone) {
      ...
      if (voice3 > 0) {
        ...
        error(1, s, "Too many voices for overlay cloning");
      }
    }
  }
  voice = p_voice - voice_tb;
  ...
  if (over_time < 0) { /* first '&' in a measure */
    ...
    for (s = p_voice->last_sym; /*s*/; s = s->prev) {
      if (s->type == BAR || s->time <= time) /* (if start of tune) */
        break;
    }
    over_time = s->time;
  } else {
    if (over_mxtime == 0)
      ...
      error(1, s, tx_wrong_dur);
      if (p_voice->time > over_mxtime)
        voice_tb[over_voice].time = over_mxtime = p_voice->time;
    }
  }
...
/* -- parse %%staves / %%score -- */
static void parse_staves(struct SYMBOL *s, struct staff_s *staves) {
  char *p;
    ...
    default:
      ...
      }
      {
        ...
        *p = '\0';
        ...
        if (v < 0) {
          error(1, s, "Voice '%s' of %%%%staves has no symbol", q);
          err = 1;
          //					break;
          p_staff = staves;
        } else {
...
static void get_staves(struct SYMBOL *s) {
  ...
  for (voice = 0, p_voice = voice_tb; voice < MAXVOICE; voice++, p_voice++) {
    p_voice->second = 0;
    p_voice->floating = 0;
    p_voice->ignore = 0;
  ...
  for (i = 0; i < MAXVOICE && p_staff->voice >= 0; i++, p_staff++) {
    voice = p_staff->voice;
  ...
  if (s->text[3] == 't') { /* if %%staves */
    for (i = 0, p_staff = staves; i < MAXVOICE - 2 && p_staff->voice >= 0;
         i++, p_staff++) {
      flags = p_staff->flags;
      if (!(flags & (OPEN_BRACE | OPEN_BRACE2)))
        continue;
      if ((flags & (OPEN_BRACE | CLOSE_BRACE)) == (OPEN_BRACE | CLOSE_BRACE) ||
          (flags & (OPEN_BRACE2 | CLOSE_BRACE2)) ==
              (OPEN_BRACE2 | CLOSE_BRACE2))
        continue;
      if (p_staff[1].flags != 0)
        continue;
      if ((flags & OPEN_PARENTH) || (p_staff[2].flags & OPEN_PARENTH))
        continue;
  ...
  staff = -1;
  for (i = 0, p_staff = staves; i < MAXVOICE && p_staff->voice >= 0;
       i++, p_staff++) {
    flags = p_staff->flags;
    ...
    p_voice->staff = p_voice->cstaff = parsys->voice[voice].staff = staff;
  ...
  for (voice = 0; voice < MAXVOICE; voice++) {
    ...
    staff = p_voice->staff;
    if (staff > 0)
      p_voice->norepbra = !(parsys->staff[staff - 1].flags & STOP_BAR);
      ...
      if (s1->abc_type == ABC_T_PSCOM &&
        ...
        while (last_staves) {
          if (last_staves->abc_type == ABC_T_PSCOM &&
              (strncmp(&last_staves->text[2], "staves", 6) == 0 ||
               strncmp(&last_staves->text[2], "score", 5) == 0)) {
...
static void set_global_def(void) {
  ...
  for (i = MAXVOICE, p_voice = voice_tb; --i >= 0; p_voice++) {
    switch (p_voice->key.instr) {
    case 0:
      if (!options.pipeformat) {
    ...
    //			p_voice->key.sf = 0;
    if (!cfmt.autoclef && p_voice->s_clef &&
...
static struct SYMBOL *get_info(struct SYMBOL *s) {
  ...
  case 'K':
    ...
    state.nbar = cfmt.measurefirst; /* measure numbering */
    over_voice = -1;
    over_time = -1;
    over_bar = 0;
    capo = 0;
  ...
  case 'V':
    ...
    if (s->state == ABC_S_TUNE && !curvoice->last_sym && curvoice->time == 0)
      voice_filter();
    break;
  case 'w':
    if (s->state != ABC_S_TUNE)
    ...
    if (!(cfmt.fields[1] & (1 << ('w' - 'a')))) {
      while (s->abc_next) {
        if (s->abc_next->abc_type != ABC_T_INFO || s->abc_next->text[0] != '+')
          break;
        s = s->abc_next;
      }
      break;
    ...
    break;
  case 'W':
    if (s->state == ABC_S_GLOBAL || !(cfmt.fields[0] & (1 << ('W' - 'A'))))
      break;
    goto addinfo;
  case 'X':
    if (!options.epsf) {
      ...
      //			else if (in_page)
        ...
        state.use_buffer = cfmt.splittune != 1;
    }
  ...
  default:
    ...
    addinfo:
      ...
      if (!prev || (prev->state == ABC_S_GLOBAL && s->state != ABC_S_GLOBAL)) {
        info[info_type - 'A'] = s;
      } else {
        while (prev->next)
          prev = prev->next;
      ...
      while (s->abc_next && s->abc_next->abc_type == ABC_T_INFO &&
             s->abc_next->text[0] == '+') {
        prev = s;
        s = s->abc_next;
        prev->next = s;
    ...
    if (s->state != ABC_S_GLOBAL)
      error(1, s, "%s info '%c:' not treated", state_txt[(int)s->state],
            info_type);
    break;
  }
  lvlarena(old_lvl);
...
void identify_note(struct SYMBOL *s, int dur, int *p_head, int *p_dots,
                   int *p_flags) {
  int head, dots, flags;
...
static void adjust_dur(struct SYMBOL *s) {
  ...
  if (s2->type == MREST || s2->type == BAR) /* in second voice */
    return;
  ...
  if (time == 0) {
    while (s2 && s2->dur == 0)
      s2 = s2->next;
    if (s2 && s2->abc_type == ABC_T_REST && (s2->flags & ABC_F_INVIS)) {
  ...
  for (; s2; s2 = s2->next) {
    ...
    s2->time = time;
    if (s2->dur == 0 || (s2->flags & ABC_F_GRACE))
      continue;
    ...
    if (s2->type != NOTEREST)
      continue;
    for (i = 0; i <= s2->nhd; i++)
      s2->u.note.notes[i].len =
          s2->u.note.notes[i].len * curvoice->wmeasure / auto_time;
    identify_note(s2, s2->u.note.notes[0].len, &head, &dots, &nflags);
    s2->head = head;
    s2->dots = dots;
    s2->nflags = nflags;
    if (s2->nflags <= -2)
...
static void get_bar(struct SYMBOL *s) {
  ...
  if (s->u.bar.repeat_bar && curvoice->norepbra && !curvoice->second)
    s->sflags |= S_NOREPBRA;
  ...
  if (s2 && s2->type == SPACE) {
    ...
    if (bar_type == B_OBRA && !s2->text &&
      ...
      s2->sflags |= s->sflags & (S_NOREPBRA | S_RBSTART | S_RBSTOP);
      s = s2;
      goto gch_build;
    }
  ...
  if (s2 && s2->type == KEYSIG && (!s2->prev || s2->prev->type != BAR)) {
    curvoice->last_sym = s2->prev;
    if (!curvoice->last_sym)
      curvoice->sym = NULL;
    sym_link(s, BAR);
  ...
  for (i = 0; i < ncmdtblt; i++) {
    ...
    if (cmdtblts[i].vn[0] != '\0') {
      if (strcmp(cmdtblts[i].vn, p_voice->id) != 0 &&
          (p_voice->nm == 0 || strcmp(cmdtblts[i].vn, p_voice->nm) != 0) &&
          (p_voice->snm == 0 || strcmp(cmdtblts[i].vn, p_voice->snm) != 0))
        continue;
    }
    tblt = tblts[cmdtblts[i].index];
    if (p_voice->tblts[0] == tblt || p_voice->tblts[1] == tblt)
      continue;
...
// fixme: 8.6.2
  ...
  for (s = parse.first_sym; s; s = s->abc_next) {
    ...
    case ABC_T_NOTE:
    case ABC_T_REST:
      if (curvoice->space && !(s->flags & ABC_F_GRACE)) {
        curvoice->space = 0;
        s->flags |= ABC_F_SPACE;
      }
      get_note(s);
    ...
    case ABC_T_CLEF:
      ...
      break;
    case ABC_T_EOLN:
      if (cfmt.breakoneoln || (s->flags & ABC_F_SPACE))
        curvoice->space = 1;
      if (cfmt.continueall || cfmt.barsperstaff ||
          s->u.eoln.type == 1) /* if '\' */
        continue;
      if (s->u.eoln.type == 0 /* if normal eoln */
          && abc2win && parse.abc_vers != (2 << 16))
        continue;
      if (parsys->voice[curvoice - voice_tb].range == 0 && curvoice->last_sym)
    ...
    case ABC_T_MREP: {
      ...
      s2 = curvoice->last_sym;
      if (!s2 || s2->type != BAR) {
        error(1, s, "No bar before measure repeat");
        break;
      }
      if (curvoice->ignore)
        break;
...
static void get_clef(struct SYMBOL *s) {
  ...
  if (is_tune_sig()) {
    ...
    s2 = p_voice->last_sym;
    if (s2 && s2->prev && s2->time == curvoice->time // if no time skip
...
static void get_key(struct SYMBOL *s) {
  ...
  if (s->u.key.sf != 0 && !s->u.key.exp && s->u.key.nacc != 0)
    set_k_acc(s);
...
--fixme
  ...
  //	s->u.key.key_delta = (cgd2cde[(s->u.key.sf + 7) % 7] + 14 +
  //s->u.key.mode) % 7;
  ...
  if (s->state == ABC_S_HEAD) { /* start of tune */
    for (i = MAXVOICE, p_voice = voice_tb; --i >= 0; p_voice++) {
      ...
      memcpy(&p_voice->ckey, &s->u.key, sizeof p_voice->ckey);
      memcpy(&p_voice->okey, &okey, sizeof p_voice->okey);
      if (p_voice->key.empty)
        p_voice->key.sf = 0;
      if (s->u.key.octave != NO_OCTAVE)
  ...
  if (is_tune_sig()) {
    ...
    memcpy(&curvoice->key, &s->u.key, sizeof curvoice->key);
    memcpy(&curvoice->ckey, &s->u.key, sizeof curvoice->ckey);
    memcpy(&curvoice->okey, &okey, sizeof curvoice->okey);
  ...
  if ((!s->abc_next ||
      ...
      && curvoice->ckey.nacc == 0 && s->u.key.nacc == 0 &&
...
static void get_meter(struct SYMBOL *s) {
  ...
  case ABC_S_TUNE:
    ...
    if (is_tune_sig()) {
      memcpy(&curvoice->meter, &s->u.meter, sizeof curvoice->meter);
      reset_gen(); /* (display the time signature) */
      break;
    }
  ...
  for (;;) {
    ...
    for (i = 1; i <= s->nhd; i++) {
      if (s->u.note.notes[i].pit >= s->u.note.notes[i - 1].pit)
        continue;
      memcpy(&v_note, &s->u.note.notes[i], sizeof v_note);
      memcpy(&s->u.note.notes[i], &s->u.note.notes[i - 1], sizeof v_note);
      memcpy(&s->u.note.notes[i - 1], &v_note, sizeof v_note);
      k = s->pits[i];
      s->pits[i] = s->pits[i - 1];
      s->pits[i - 1] = k;
      k = new_order[i];
...
static void set_map(struct SYMBOL *s) {
  ...
  for (m = 0; m <= s->nhd; m++) {
    ...
    for (note_map = map->notes; note_map; note_map = note_map->next) {
      ...
      case MAP_ONE:
        if (note->pit == note_map->pit && note->acc == note_map->acc)
          break;
        continue;
      case MAP_OCT:
...
static void get_note(struct SYMBOL *s) {
  ...
  if (curvoice->perc)
    s->sflags |= S_PERC;
  else if (s->abc_type == ABC_T_NOTE && curvoice->transpose != 0)
    note_transpose(s);
  ...
  if (!(s->flags & ABC_F_GRACE)) {
    ...
    if (curvoice->key.instr != K_HP && curvoice->key.instr != K_Hp &&
        !options.pipeformat) {
      div = 2;
      if (!prev || !(prev->flags & ABC_F_GRACE)) {
  ...
  if (s->abc_type == ABC_T_REST) {
    ...
    /* sort the notes of the chord by pitch (lowest first) */
    if (!(s->flags & ABC_F_GRACE) && curvoice->map_name)
      set_map(s);
    sort_pitch(s);
  }
    ...
    if ((l = s->u.note.notes[0].len) != 0) {
      ...
      for (i = 1; i <= m; i++) {
        if (s->u.note.notes[i].len == l)
          continue;
        identify_note(s, s->u.note.notes[i].len, &head, &dots, &nflags);
        if (head > s->head)
          s->head = head;
        if (dots > s->dots)
          s->dots = dots;
...
static char *get_val(char *p, float *v) {
  ...
  while (isspace((unsigned char)*p))
    p++;
  while ((isdigit((unsigned char)*p) && r < &tmp[32 - 1]) || *p == '-' ||
         *p == '.')
    *r++ = *p++;
  *r = '\0';
  sscanf(tmp, "%f", v);
  ...
  *r++ = '/';
  ...
  if (scale || trans) {
    ...
    if (!scale)
      ...
      r += sprintf(r, "%.2f %.2f T %.2f %.2f scale ", x, -y, x1, y1);
    else
      r += sprintf(r, "%.2f %.2f scale %.2f %.2f T ", x1, y1, x, -y);
  }
  strcpy(r, "0 0 M\n");
  r += strlen(r);
  if (width && width < q) {
  ...
  for (;;) {
    ...
    switch (*p++) {
    default:
      if ((isdigit((unsigned char)p[-1])) || p[-1] == '-' || p[-1] == '.') {
        if (!npar)
          continue;
        p--; // same op
        break;
    ...
    case 'q':
      ...
      t = get_val(p, &x);
      t = get_val(t, &y);
      r += sprintf(r, " %.2f %.2f %.2f %.2f", x1 * 2 / 3, -y1 * 2 / 3,
    ...
    *r++ = ' ';
    ...
    r += strlen(r);
    if (r + 30 > rmax)
      bug("Buffer overflow in SVG to PS", 1);
  }
...
static void ps_def(struct SYMBOL *s, char *p, char use) /* cf user_ps_add() */
{
  if (!options.svg && options.epsf <= 1) { /* if PS output */
    if (options.secure
        //		 || use == 'g'		// SVG
        || use == 's') // PS for SVG
      return;
  ...
  if (options.file_initialized > 0 || mbf != outbuf)
    a2b("%s\n", p);
  else
    user_ps_add(p, use);
}
...
static char *get_symsel(struct symsel_s *symsel, char *p) {
  ...
  if (*q >= 'a' && *q <= 'z')
    ...
    symsel->seq = 0;
  if (*q == ':') {
    if (sscanf(q + 1, "%d/%d%n", &tn, &td, &n) != 2 || td <= 0)
      return 0;
...
static int get_color(char *p) {
  ...
  if (*p == '#') {
    if (sscanf(p, "#%06x", &color) != 1 || (unsigned)color > 0x00ffffff)
      return -1;
    return color;
  }
  for (i = sizeof col_tb / sizeof col_tb[0]; --i >= 0;) {
    if (strncasecmp(p, col_tb[i].name, strlen(col_tb[i].name)) == 0)
      break;
  }
  if (i < 0)
    return -1;
...
static void get_map(char *p) {
  ...
  for (note_map = map->notes; note_map; note_map = note_map->next) {
    if (note_map->type == type && note_map->pit == pit && note_map->acc == acc)
      break;
  }
  if (!note_map) {
...
static struct SYMBOL *process_pscomment(struct SYMBOL *s) {
  ...
  p = s->text + 2; /* skip '%%' */
  q = p + strlen(p) - 5;
  if (q > p && strncmp(q, " lock", 5) == 0) {
    lock = 1;
    *q = '\0';
  }
  p = get_str(w, p, sizeof w);
  if (s->state == ABC_S_HEAD && !check_header(s)) {
    error(1, s, "Cannot have %%%%%s in tune header", w);
    return s;
  }
  switch (w[0]) {
  case 'b':
    if (strcmp(w, "beginps") == 0 || strcmp(w, "beginsvg") == 0) {
      char use;
    ...
    if (strcmp(w, "begintext") == 0) {
      ...
      if (s->state == ABC_S_TUNE) {
        ...
        if (options.epsf || !options.in_fname)
          return s;
      }
      p = s->text + 2 + 9;
      while (*p == ' ' || *p == '\t')
  ...
  case 'c':
    ...
    if (strcmp(w, "clip") == 0) {
      ...
      if (clip_start.bar < 0)
        clip_start.bar = 0;
      if (clip_end.bar < clip_start.bar || (clip_end.bar == clip_start.bar &&
  ...
  case 'E':
    if (strcmp(w, "EPS") == 0) {
      ...
      gen_ly(1);
      if (options.secure || cfmt.textoption == T_SKIP)
        return s;
      ...
      while (fgets(line, sizeof line, fp)) {
        if (strncmp(line, "%%BoundingBox:", 14) == 0) {
          if (sscanf(&line[14], "%f %f %f %f", &x1, &y1, &x2, &y2) == 4)
            break;
        }
      }
      fclose(fp);
      ...
      if (cfmt.textoption == T_CENTER || cfmt.textoption == T_RIGHT) {
        float lw;
    ...
    break;
  case 'g':
    if (strcmp(w, "gchord") == 0 || strcmp(w, "gstemdir") == 0) {
      set_voice_param(curvoice, s->state, w, p);
      return s;
    }
    if (strcmp(w, "glyph") == 0) {
  ...
  case 'm':
    ...
    if (strcmp(w, "multicol") == 0) {
      ...
      if (strncmp(p, "start", 5) == 0) {
        ...
        if (multicol_start == 0) {
          error(1, s, "%%%%%s new without start", w);
        } else {
          buffer_eob(0);
          bposy = get_bposy();
        ...
        if (multicol_start == 0) {
          error(1, s, "%%%%%s end without start", w);
        } else {
          buffer_eob(0);
          bposy = get_bposy();
          if (bposy > multicol_max)
          ...
          multicol_start = 0;
          buffer_eob(0);
          if (!info['X' - 'A'] && !options.epsf)
            write_buffer();
        }
      } else {
        error(1, s, "Unknown keyword '%s' in %%%%%s", p, w);
      }
      return s;
    }
    break;
  case 'M':
    if (strcmp(w, "MIDI") == 0 && strncmp(p, "temperamentequal", 16) == 0) {
      int n;
  ...
  case 'p':
    ...
    if (strcmp(w, "ps") == 0 || strcmp(w, "postscript") == 0) {
      ps_def(s, p, 'b');
      return s;
    }
    break;
  ...
  case 'r':
    if (strcmp(w, "repbra") == 0) {
      if (s->state != ABC_S_TUNE)
        return s;
      curvoice->norepbra = strchr("0FfNn", *p) || *p == '\0';
      return s;
    }
    if (strcmp(w, "repeat") == 0) {
      ...
      if (s->state != ABC_S_TUNE)
        return s;
      if (!curvoice->last_sym) {
        error(1, s, "%%%s cannot start a tune", w);
        return s;
      }
      if (*p == '\0') {
        n = 1;
        ...
        n = atoi(p);
        if (n < 1 || (curvoice->last_sym->type == BAR && n > 2)) {
          error(1, s, "Incorrect 1st value in %%%%%s", w);
          return s;
        }
        while (*p != '\0' && !isspace((unsigned char)*p))
          p++;
        ...
        if (*p == '\0') {
          ...
          k = atoi(p);
          if (k < 1) {
            //					 || (curvoice->last_sym->type ==
            //BAR
            //					  && n == 2
            //					  && k > 1)) {
            error(1, s, "Incorrect 2nd value in %%%%%s", w);
            return s;
          }
        }
      }
  ...
  case 's':
    ...
    if (strcmp(w, "staves") == 0 || strcmp(w, "score") == 0) {
      if (s->state == ABC_S_GLOBAL)
        return s;
      get_staves(s);
      return s;
  ...
  case 't':
    ...
    if (strcmp(w, "tablature") == 0) {
      ...
      case ABC_S_HEAD:
        ...
        /* !! 2 tblts per voice !! */
        if (curvoice->tblts[0] == tblt || curvoice->tblts[1] == tblt)
          break;
        if (curvoice->tblts[1]) {
          error(1, s, "Too many tablatures for voice %s", curvoice->id);
          break;
        }
        if (!curvoice->tblts[0])
    ...
    if (strcmp(w, "transpose") == 0) {
      ...
      case ABC_S_HEAD: {
        ...
        for (i = MAXVOICE, p_voice = voice_tb; --i >= 0; p_voice++) {
          p_voice->transpose = cfmt.transpose;
          memcpy(&p_voice->key, &p_voice->okey, sizeof p_voice->key);
          key_transpose(&p_voice->key);
          memcpy(&p_voice->ckey, &p_voice->key, sizeof p_voice->ckey);
          if (p_voice->key.empty)
            p_voice->key.sf = 0;
        }
        return s;
      ...
      s2 = curvoice->sym;
      if (!s2) {
        memcpy(&curvoice->key, &curvoice->okey, sizeof curvoice->key);
        key_transpose(&curvoice->key);
        memcpy(&curvoice->ckey, &curvoice->key, sizeof curvoice->ckey);
        if (curvoice->key.empty)
          curvoice->key.sf = 0;
        return s;
      }
      ...
      memcpy(&s2->u.key, &curvoice->okey, sizeof s2->u.key);
      key_transpose(&s2->u.key);
      memcpy(&curvoice->ckey, &s2->u.key, sizeof curvoice->ckey);
      if (curvoice->key.empty)
        s2->u.key.sf = 0;
      return s;
    }
  ...
  if (s->state == ABC_S_TUNE) {
    if (strcmp(w, "leftmargin") == 0 || strcmp(w, "rightmargin") == 0 ||
        strcmp(w, "scale") == 0) {
      generate();
      block_put();
    }
